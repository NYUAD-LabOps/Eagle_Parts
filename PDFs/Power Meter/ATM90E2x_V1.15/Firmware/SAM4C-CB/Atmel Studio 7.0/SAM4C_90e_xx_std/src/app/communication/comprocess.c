...
{	  
	...
	if ( ptr->state == CMDPROCESS || ptr->state == SEND || (!( ul_status & UART_SR_RXRDY )) )
    {        
        return;
    }
...
{	  
	...
	if( ptr->state ==CMDPROCESS || ptr->state ==SEND || (!( ul_status & US_CSR_RXRDY )) )
	{		        
		return;
	}
	switch(ptr->protocolmode)
...
void	Init_Usart_App(UART_TYPE uart_id)
{
	...
	{		
		case	ZigBee_ID:
		{
			//---configure USART2_RXD/TXD/RTS to peripheral_A -------
			pio_configure( ZigBee_COM_PIO, ZigBee_COM_TYPE, ZigBee_COM_MASK, ZigBee_COM_ATTR );
			ZigBee_COM_CTS_PIO_En();
			ZigBee_COM_CTS_PullUp_En();
			ZigBee_COM_CTS_Out();
			ZigBee_COM_CTS_Low();			//usart2 is receive status
		
			pmc_enable_periph_clk(ZigBee_COM_ID);
		...
		case	PLC_ID:
		{
			//---configure USART3_RXD/TXD/RTS to peripheral_A -------
			pio_configure( PLC_COM_PIO, PLC_COM_TYPE, PLC_COM_MASK, PLC_COM_ATTR );
			PLC_COM_CTS_PIO_En();
			PLC_COM_CTS_PullUp_En();
			PLC_COM_CTS_Out();
			PLC_COM_CTS_Low();			//usart3 is receive status
		
			pmc_enable_periph_clk( PLC_COM_ID );
	...
	switch( uart_id )
	{
		case	ZigBee_ID:
		{
			NVIC_EnableIRQ( ZigBee_COM_IRQn );
		}
		break;		
		case	PLC_ID:
		{
			NVIC_EnableIRQ( PLC_COM_IRQn );
		}
		break;
	}	
}
...
void	Init_Uart_App(UART_TYPE uart_id)
{
	...
	switch( uart_id )
	{
		case	Rs485_ID:
		{
			...
			pio_configure( Rs485_COM_PIO, Rs485_COM_TYPE, Rs485_COM_MASK, Rs485_COM_ATTR );
            pmc_enable_periph_clk( Rs485_COM_ID );
		}
		break;
		case	Ir_ID:
		{
			...
			pio_configure( IR_COM_PIO, IR_COM_TYPE, IR_COM_MASK, IR_COM_ATTR );
            pmc_enable_periph_clk( IR_COM_ID );

			//PMC->CKGR_PLLAR =0x00FA6401;	//mul=250,count=100,en=1
			//while( (PMC->PMC_SR & PMC_SR_LOCKA)==0 );
			//---Modulation Clock = 8192/(8*(19+8)) =37.926 kHz
			mode |=UART_MR_OPT_CLKDIV(19) | UART_MR_OPT_DUTY_DUTY_50 | UART_MR_OPT_MDINV_DISABLED | UART_MR_OPT_EN_ENABLED; 
			//UART1->UART_MR |=UART_MR_OPT_CLKDIV(19) | UART_MR_OPT_DUTY_DUTY_50 | UART_MR_OPT_MDINV_DISABLED | UART_MR_OPT_EN_ENABLED; 
		}
		break;
	}//
	i =	sysclk_get_cpu_hz();
	...
	*/
	...
	uart_enable_interrupt(Uartx, UART_IER_RXRDY);
	if( uart_id==Rs485_ID )
	{
		NVIC_EnableIRQ(Rs485_COM_IRQn);
	}
	if( uart_id==Ir_ID )
	{
...
void	ComSendProcess(UART_TYPE uart_id)
{
	...
	if( uart_id== Ir_ID )
	{
		ptr =&VCom.IR;
		//--------------modulation send--------------------
        IR_COM->UART_MR |=UART_MR_OPT_CLKDIV(12) | UART_MR_OPT_DUTY_DUTY_50 | UART_MR_OPT_MDINV_DISABLED; 
		IR_COM->UART_MR |=UART_MR_OPT_EN_ENABLED; 
		//-------------------------------------------------		
	}
	if( uart_id== PLC_ID )
	{
	...
	if( ptr->state ==SEND )
	{
		...
		{	
			switch(uart_id)
			{
				case	Rs485_ID:
				{
					uart_write( Rs485_COM,ptr->buff[ptr->offset++] );
					while( !(uart_get_status(Rs485_COM) & UART_SR_TXRDY) );
					sendid =Rs485Transmit;
				}
				break;
				case	Ir_ID:
				{
					uart_write( IR_COM,ptr->buff[ptr->offset++] );
					while( !(uart_get_status(IR_COM) & UART_SR_TXRDY) );
					sendid =IrTransmit;
				}
				break;
				case	ZigBee_ID:
				{
					ZigBee_COM_CTS_High();		//usart2 is send status
					usart_write( ZigBee_COM,ptr->buff[ptr->offset++] );
					while( !(usart_get_status( ZigBee_COM ) & US_CSR_TXRDY) );
					sendid =ZigBeeTransmit;
				}
				break;					
				case	PLC_ID:
				{
					PLC_COM_CTS_High();		//usart3 is send status
					usart_write( PLC_COM,ptr->buff[ptr->offset++] );
					while( !(usart_get_status( PLC_COM ) & US_CSR_TXRDY) );
					sendid =PLCTransmit;						
				}
				break;
			}	
