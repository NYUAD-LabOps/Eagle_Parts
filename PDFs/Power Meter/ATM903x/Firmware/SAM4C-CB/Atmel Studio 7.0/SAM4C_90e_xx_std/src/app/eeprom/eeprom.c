...
void	BSP_E2promBus_Init(void)
{
	...
	pmc_enable_periph_clk(EEPROM_SDA_ID);
	pmc_enable_periph_clk(EEPROM_SCL_ID);
	I2C_SDA_PIO_En();
	I2C_SCL_PIO_En();
	I2C_SDA_PullUp_En();
	I2C_SCL_PullUp_En();
...
{
	uint8_t	WrDeviceAddr;
	uint16_t RdLen;
	uint8_t	RdE2PgInAdr,RdE2PgAdr;
	uint8_t 	count,len;
	uint8_t	ptroffset=0;	
	...
	RdE2PgInAdr =(uint8_t)(offset);
	RdE2PgAdr =(uint8_t)(offset>>8);
	RdLen =0;
	while(size>0)
	{	
		...
		*(buffer+count+ptroffset)=Read_I2C_1ByteNoAck();
		...
		size=size-len;
		RdLen=RdLen-len;
		if((uint8_t)(RdE2PgInAdr+len)<RdE2PgInAdr)
		{
			RdE2PgAdr++;
			if(RdE2PgAdr==0)
			{
...
uint16_t	EEPROMwriter(uint32_t offset,uint8_t *buffer,uint16_t size)
{
	uint8_t	WrDeviceAddr;
	uint16_t WrLen;
	uint8_t	WrE2PageAddr,WrE2PageInAddr;
	uint8_t 	count,len;
	uint8_t	ptroffset=0;
	WrLen	=0;
	WrDeviceAddr =WrE2_DeviceAddr;
	...
	WrE2PageAddr =(uint8_t)(offset>>8);
	WrE2PageInAddr =(uint8_t)(offset);
	...
	{	
		...
		size=size-len;
		WrLen=WrLen+len;
		if((uint8_t)(WrE2PageInAddr+len)<WrE2PageInAddr)
		{
			WrE2PageAddr++;
			if(WrE2PageAddr==0)
			{
...
//Effect          ::
//===============================================
uint8_t	RdE2DataCRC(uint16_t E2PgInAdr,uint8_t *ptr,uint8_t len)
{
	uint8_t	i;
	uint8_t	Buff[16];
	...
	for(i=0;i<3;i++)
	{
		...
		COPYBYTEDATA((&Buff[0]+((uint8_t)(E2PgInAdr)&0x0F)),ptr,len);//copy data
		break;
	}
...
//Effect          ::
//===============================================
uint8_t	WrE2DataCRC(uint16_t E2PgInAdr,uint8_t *ptr,uint8_t len)
{
	uint8_t	i;
	uint8_t	Buff[16];
	if(len ==0 || len >14 || ((E2PgInAdr&0x000F) +len)>15)
	...
	COPYBYTEDATA(ptr,(&Buff[0]+((uint8_t)(E2PgInAdr)&0x0F)),len);//copy data
